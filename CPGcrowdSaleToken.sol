...
contract CPGCrowdSale is Ownable {
  ...
  event ChangeSaleState(uint mainSaleStartTime, uint mainSaleEndTime, uint8 currentState);
  ...
  function buyTokens(address beneficiary) public payable {
    ...
    // 限制ether小数点
    require(msg.value >= mileStones[currentState].saleMinNumEther * 1 ether);
    require(msg.value <= mileStones[currentState].saleMaxNumEther * 1 ether);
  ...
  function setMainSaleDates(uint _mainSaleStartTime, uint _mainSaleEndTime, uint8 _currentState, bool clearStats) public onlyOwner returns (bool) {
    ...
    if (clearStats) {
      for (uint i = _currentState; i < mileStones.length; i++) {
        mileStones[i].amountEtherRaised = 0;
      }
    }
    ChangeSaleState(mainSaleStartTime, mainSaleEndTime, currentState);
    return true;
  }
